<form method="post" class="ajax-screen-submit" autocomplete="off" action="{{ action('SystemUserController@update', $id) }}" enctype="multipart/form-data">
	...
	<div class="row p-2">
		...
		<div class="col-md-12">
			<div class="form-group">
				<label class="control-label">{{ _lang('User Department') }}</label>
				<select class="form-control select2-ajax" data-href="{{ route('departments.create') }}" data-title="{{ _lang('Add New department') }}" data-value="id" data-display="name" data-table="departments" id="department" name="department">
					<option value="">{{ _lang('Select One') }}</option>
